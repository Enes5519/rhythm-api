...
const getVideoList = async (keyword) => {
    try {
        const response = await fetch("https://www.youtube.com/results?q=" + encodeURIComponent(keyword));
        ...
        return parseJsonFormat(data.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents)
    } catch (ex) {
